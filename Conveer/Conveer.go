...
type Box struct {
	...
	boxData3 []string //Какие то данные которые нужны для бизнеса
	process *mutexRunner  // Системная функция конвеера, для открытия нового миньона без гонок
	ch <- chan string  // Приемник данных, с которыми сервис будет работать
	OutCh chan string // Выходные даннае можно отправлять в следующий конвеер этим каналом
...
func NewBalancer(boxData string,boxData2 string,boxData3 []string,ch <- chan string) *Box { // <- Вход в библиотеку тут!
	...
	return &Box{
		...
		boxData2:boxData2, //Присваеваем входные данные для использования всем миньонам
		boxData3:boxData3, //Присваеваем входные данные для использования всем миньонам
		...
		OutCh:make(chan string, 10000), // Инициализируем буферный канал для вывода с буфером в 10000 стрингов свободного ожидания, при переполнении работа будет ожидать.
...
func (p *Box) minion(gophere int)  {
	 ...
	 */
	var counter int // К примеру можно открыть счетчик
	for  { // Начинается тело сканирования каналов, из цикла выходить не планируем, завершение цикла будет завершение функции
		...
		case elem := <-p.ch: // Канал данных, с которыми нужно проводить какие то операции, к примеру строки или уже обработаные другим конвеером строки
			...
			p.OutCh <- newElement // Отправляем на вывод, новую строку. С ней может работать другой конвеер, который делает, что то другое
			if counter >= 50 { // Когда счетчик дошел до какого то колличества, можем это как то использовать
				fmt.Printf("Мы достигли %d операций, все идет хорошо, продолжаю работу, я это миньен %d, имя: %s | %s\n",counter,gophere,p.boxData,newElement) //Печатаем ход работ. И в тексте добавляем полезную информацию, о количестве завершеных им операций и порядковый номер миньона
		...
		default: // Попадаем сюда когда нет работы, канал пуст и тут можем узнать, может пора закругляться
			if p.stopPrepare.x { // Функции сверху обрабатывают два типа сигнала, это по прихода из канала или просто вызов Stop, и они передают сюда true, значит нам пора выключаться
				...
				fmt.Printf("Мы достигли %d операций, и я завершаю работу, я это миньен %d, имя: %s\n",counter,gophere,p.boxData) //Печатаем ход работ. И в тексте добавляем полезную информацию, о количестве завершеных им операций и порядковый номер миньона
